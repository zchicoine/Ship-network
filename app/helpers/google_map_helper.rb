...
module GoogleMapHelper
    ...
    def get_ship_details_first_table ship_name = "null"
        ...
        @hash_is_nil = true
        @return_ship_details.as_json.each do |k, v|
          unless v.blank? || v == 0
            @hash_is_nil = false
          end
        end

        @return_ship_details_table_one = [@return_ship_details, @hash_is_nil]
    end
    ...
    def get_ship_details_third_table ship_name = "null"
      @ship_id_two = Ship.find_by_name(ship_name).id
      @return_ship_details_three = ShipDetail.select(:marine_diesel_oil?, :laden, :ballast, :economic,
